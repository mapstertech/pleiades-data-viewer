  ...
  const geocoder = new MapboxGeocoder({
    ...
    mapboxgl: mapboxgl,
    placeholder : "Search for a modern-day area to zoom in closer."
  })
  ...
  map.on('load', () => {
    ...
    map.on('mousemove', (e) => {
      const features = map.queryRenderedFeatures(e.point, { layers : ['pleiades_places_areas', 'pleiades_places', 'pleiades_places_nature'] });
    ...
    map.on('click', (e) => {
      ...
      if(features.length >  0) {
        ...
        for(let prop in features[0].properties) {
          html += `<tr><td>${prop}</td><td>${features[0].properties[prop]}</td></tr>`
        }
        html += "</tbody></table>";
        $('#more-info').html(html)
